...
public class TicketSystem {
	...
	public static void answerTicket(Player player) {
		...
		filterTickets();
		if (tickets.isEmpty()) {
			player.getPackets().sendGameMessage("There are currently no tickets available. Keep it up!");
			return;
		...
		while(tickets.size() > 0) {
			...
			if(target.isLocked() || target.getControlerManager().getControler() instanceof RuneDungGame){
			player.getPackets().sendGameMessage("The player is in Dungeoneering.... Tisk Tisk!");
			break;
			}
			if (target == null) 
