...
public class PlayerCombat extends Action {
	...
	public boolean start(Player player) {
		...
		weaponName = ItemDefinitions.getItemDefinitions(player.getEquipment().getWeaponId())
		.getName().toLowerCase();
		
		if(weaponName.contains("2h") || weaponName.contains("godsword") || weaponName.contains("noxious scythe") || weaponName.contains("halberd")){
			player.getAppearence().setRenderEmote(2689); //hally
		}
		else if(weaponName.contains("2h") || weaponName.contains("godsword")){
			player.getAppearence().setRenderEmote(2692);
		}
		else if(weaponName.contains("scimitar") || weaponName.contains("drygore") || weaponName.contains("longsword") || weaponName.contains("whip")
				|| weaponName.contains("darklight") || weaponName.contains("brackish blade") || weaponName.contains(" sword")){
			player.getAppearence().setRenderEmote(2687);//swords render
		}
		else if(weaponName.contains("bow") && !weaponName.contains("cross")){
			player.getAppearence().setRenderEmote(2691);//bow render
		}
		else if(weaponName.contains("crossbow")){
			player.getAppearence().setRenderEmote(2694); //cbow render
		}
		player1 = player;
		if (checkAll(player)) {
			return true;
	...
	private int rangeAttack(final Player player) {
		...
		if (player.getCombatDefinitions().isUsingSpecialAttack()) {
			...
			if (weaponId != -1) {
				...
				if (weaponName.contains("throwing axe")
				...
				} else if (weaponName.contains("crossbow") || weaponName.contains("quick-fire") || weaponName.contains("Quick-Fire")) {
					int damage = 0;
					int ammoId = player.getEquipment().getAmmoId();
					if (ammoId != -1 && Utils.getRandom(10) == 5) {
						...
						case 9242:
							...
							if (target.getHitpoints() > 5000) {
								damage = (int) (target.getHitpoints() * 0.06);
							} else {
					...
					if (!player.isDonator() && !player.isExtremeDonator()) {
						player.getPackets().sendGameMessage("You have to be VIP to be using Bolas.");
						return combatDelay;
					}
					dropAmmo(player, -3);
			...
			if (weaponId != -1) {
				if ((quantity == -3 && Utils.getRandom(10) < 2) || (quantity != -3 && Utils.getRandom(3) > 0)) {
					...
					if (capeId != -1
							...
							.getName().contains("Ava's") || weaponId == 25202)
		...
		if (player.getCombatDefinitions().isUsingSpecialAttack()) {
			...
			case 29969:
				...
				if (hit1 > 0) {
					...
					hit1 = getRandomMaxHit(player, weaponId, attackStyle, false, true, 1.0, true);
					if (hit1 > 0) {
				...
				for (int i = 0; i < hits1.length; i++) {
					if (i > 1) {
						delayHit(1, weaponId, attackStyle, getMeleeHit(player, hits1[i]));
					} else {
						delayNormalHit(weaponId, attackStyle, getMeleeHit(player, hits1[i]));
					}
				}
				break;
			case 10887: // anchor
			...
			case 4587: // dragon sci
				...
				delayNormalHit(weaponId, attackStyle, hit11);
				soundId = 2540;
				break;
		...
		if (defenceAffects) {
			...
			if (target instanceof Player) {
				...
				if(n.getId() == 1160 && fullVeracsEquipped(player))
					def *= 0.6;
				if (player.finalblow) {
					if (n.getHitpoints() < (n.getHitpoints() / 2)) {
						if (player.getSkills().getAssassinLevel(Skills.FINAL_BLOW) >= 10 && player.getSkills().getAssassinLevel(Skills.FINAL_BLOW) <= 19)
							def *= 0.95;
						else if (player.getSkills().getAssassinLevel(Skills.FINAL_BLOW) >= 20 && player.getSkills().getAssassinLevel(Skills.FINAL_BLOW) <= 29)
							def *= 0.925;
						else if (player.getSkills().getAssassinLevel(Skills.FINAL_BLOW) >= 30 && player.getSkills().getAssassinLevel(Skills.FINAL_BLOW) <= 39)
							def *= 0.9;
						else if (player.getSkills().getAssassinLevel(Skills.FINAL_BLOW) >= 40 && player.getSkills().getAssassinLevel(Skills.FINAL_BLOW) <= 49)
							def *= 0.85;
						else if (player.getSkills().getAssassinLevel(Skills.FINAL_BLOW) >= 50 && player.getSkills().getAssassinLevel(Skills.FINAL_BLOW) <= 59)
							def *= 0.8;
						else if (player.getSkills().getAssassinLevel(Skills.FINAL_BLOW) >= 60 && player.getSkills().getAssassinLevel(Skills.FINAL_BLOW) <= 69)
							def *= 0.75;
						else if (player.getSkills().getAssassinLevel(Skills.FINAL_BLOW) >= 70 && player.getSkills().getAssassinLevel(Skills.FINAL_BLOW) <= 79)
							def *= 0.7;
						else if (player.getSkills().getAssassinLevel(Skills.FINAL_BLOW) >= 80 && player.getSkills().getAssassinLevel(Skills.FINAL_BLOW) <= 89)
							def *= 0.64;
						else if (player.getSkills().getAssassinLevel(Skills.FINAL_BLOW) >= 90 && player.getSkills().getAssassinLevel(Skills.FINAL_BLOW) <= 98)
							def *= 0.56;
						else if (player.getSkills().getAssassinLevel(Skills.FINAL_BLOW) == 99)
							def *= 0.5;
					}
				}
			}
	...
	public static int getWeaponAttackEmote(int weaponId, int attackStyle) {
		int shieldId = player1.getEquipment().getShieldId();
		if (weaponId != -1) {
			if (weaponId == 28011) {
				 return 4230;
			}
			...
			//player1.sendMessage(shieldName);
			if (weaponName != null && !weaponName.equals("null")) {
				if(shieldName.contains("off-hand") && shieldName.contains("cross") && weaponName.contains("cross") && Utils.random(2) == 0 )
					return 18231;
				...
				if(shieldName.contains("off-hand") && Utils.random(2) == 0 && !shieldName.contains("cross") && !shieldName.contains("rapier") && !weaponName.contains("staff")){
					return 18244;
				}
				if(shieldName.contains("off-hand") && Utils.random(2) == 0 && !shieldName.contains("cross") && shieldName.contains("rapier") && !weaponName.contains("staff"))
					return 18239;
				if(weaponName.contains("rapier") || weaponName.contains("brackish"))
					return 18225;
				if(weaponName.contains("drygore longsword") || weaponName.contains("drygore mace"))
					return 18221;
	...
	public void stop(final Player player) {
		player.getAppearence().setRenderEmote(player.getAppearence().getRenderEmote());
			player.getAppearence().setRenderEmote(2912);
		//player.sendMessage("render : "+player.getAppearence().getRenderEmote() );
		player.setNextFaceEntity(null);
	}
	...
	public static final int isRanging(Player player) {
		...
		case 28437: // asc crossbow
			switch (ammoId) {
			case -1:
				return 3;
			case 28465: // Asc bolts
				return 2;
			default:
				return 1;
			}
		case 33318: // asc crossbow
			switch (ammoId) {
			case -1:
				return 3;
			case 28465: // Asc bolts
