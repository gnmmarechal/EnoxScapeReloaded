...
public class MartintheMasterGardener extends Dialogue {
	
	public static int SKILLCAPE = 9811;
	public static int SKILLHOOD = 9812;
		...
		npcId = (Integer) parameters[0];
			sendEntityDialogue(
				...
				new String[] {
						...
						"buy the Farming Skillcape?"}, IS_NPC, npcId,
				9760);
		stage = -1;
	}
	...
	public void run(int interfaceId, int componentId) {
		...
		if (componentId == OPTION_1) {
			if (player.getSkills().getLevelForXp(Skills.FARMING) < 99) {
				player.getPackets()
						.sendGameMessage(
								"You need a Farming level of 99.");
				return;
			}
