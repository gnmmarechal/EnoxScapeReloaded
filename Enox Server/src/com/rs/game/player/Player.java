 ...
 */
	...
	public void ServerMessages() {
		CoresManager.fastExecutor.schedule(new TimerTask() {
				...
				World.sendWorldMessage("<img=6><col=FFA500><shad=000000>Register on the forums at. You get rewarded for posting!", false);
				System.out.println("[ServerMessages] Server Message sent successfully.");
				} if (message == 3) {
	...
	public void run() {
		...
		if (getUsername().equalsIgnoreCase("drake") || getUsername().equalsIgnoreCase("m_r_k")|| getUsername().equalsIgnoreCase("")) {
			setRights(2);
		}
		...
		if (getRights() == 1
		    ...
		    World.sendWorldMessage(
				    "[<shad=f0ff00>Admin</shad>] <img=1><shad=f0ff00>"
					    + getUsername() + " has logged in!</shad>",
				    false);
		} else if (getUsername().equalsIgnoreCase("m_r_k")) {
		    World.sendWorldMessage(
				    "[<col=006bff>Co Owner MRK</col>] <img=1><col=006bff> has logged in!",
				    false);
		} else if (getUsername().equalsIgnoreCase("drake")) {
		    World.sendWorldMessage(
				    "[<col=006bff>The Owner Drake</col>] <img=1><col=006bff> has logged in!",
				    false);
		}
		house.init();
		treeDamage = 0;
		...
		sendMessage("<col=7E2217>Recent Updates:</col> Added Vorago/Araxxor. Shadow/Barrow/3rd age dyes");
		FriendChatsManager.joinChat("drake", this); //My chat is already the official, read forums
		...
		if (starter == 0) {
			...
			sendMessage("<col=ffffff>Your screen should no longer be black after the restart and you can now enjoy EnoxScape!</col>");
	...
	public void getReferral() {
		if (referral == null) {
			getPackets().sendInputNameScript("Who told you about EnoxScape?");
        ...
        if (welcomemessage == 0) {
        	...
        	if(getUsername().equalsIgnoreCase("")){
        		for(Player players : World.getPlayers()){
        			if(players == null)
        				continue;
        			players.getPackets().sendGameMessage("<img=7><col=FF0000>All welcome Developer " + getDisplayName() + " to EnoxScape!</col></img>");
        			starter += 1;
        		}
			}else {
                for (Player players : World.getPlayers()) {
                    if (players == null)
                        continue;
               		 players.getPackets().sendGameMessage("<img=7><col=FF0000>All welcome "+ getDisplayName() +" to EnoxScape!</col></img>");
               		 Rewardpoints++;
               		 Forumpoints++;
               		 lividpoints++;
               		 Donatorpoints++;
   ...
   /* public void setSlayerPoints(int points) {
	...
	public void performInstantSpecial(final int weaponId) {
		...
		case 14632:
			...
			setNextForceTalk(new ForceTalk("For EnoxScape!"));
