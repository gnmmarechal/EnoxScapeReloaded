...
public class DivineHerblore extends Action {
	...
	public int processWithDelay(Player player) {
		...
		if (manager4.getTask() != null) {
			if(spot.getHerbs()[fishId].getId() == player.TASK) {
		...
		int roll = Misc.random(100);
		 Player owner = spott.getOwner();
			if(player == owner && roll >= 60) {
				...
				owner.sendMessage(""+player.getUsername()+" harvested some "+fishName+" for you.");	
				player.getInventory().addItem(fish);
	...
	private boolean checkAll(Player player) {
		if (player.getSkills().getLevel(Skills.HERBLORE) < spot.getHerbs()[fishId].getLevel()) {
			player.getDialogueManager().startDialogue("SimpleMessage","You need a herblore level of " + spot.getHerbs()[fishId].getLevel() + " to harvest here.");
			return false;
		}
