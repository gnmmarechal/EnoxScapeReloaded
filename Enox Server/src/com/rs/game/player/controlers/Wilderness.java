		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				if (loop == 0) {
					...
					try {
					if (player.isPker || killer.isPker)
						World.addGroundItem(Player.pvpRandom(), player.getLastWorldTile(), killer, true, 2000000000);
					} catch (Exception e) {
					    // Whatever
					}
					if (killer != null) {
						...
						try {
							BufferedWriter bf = new BufferedWriter(new FileWriter(
									Settings.LOG_PATH + "pking/kills.txt", true));
							bf.write("["+ killer.getDisplayName() +", "+ DateFormat.getDateTimeInstance().format(new Date())
								+ "]: has killed the player "+ player.getDisplayName() + ".");
							bf.newLine();
							bf.flush();
							bf.close();
