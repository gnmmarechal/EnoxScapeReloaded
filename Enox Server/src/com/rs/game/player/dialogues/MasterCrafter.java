...
public class MasterCrafter extends Dialogue {
	
	public static int SKILLCAPE = 9781;
	public static int SKILLHOOD = 9782;
		...
		npcId = (Integer) parameters[0];
			sendEntityDialogue(
				...
				new String[] {
						...
						"buy the Crafting Skillcape?"}, IS_NPC, npcId,
				9760);
		stage = -1;
	}
	...
	public void run(int interfaceId, int componentId) {
		...
		if (componentId == OPTION_1) {
			if (player.getSkills().getLevelForXp(Skills.CRAFTING) < 99) {
				player.getPackets()
						.sendGameMessage(
								"You need a Crafting level of 99.");
				return;
			}
