...
public final class WorldPacketsDecoder extends Decoder {
	...
	public static void decodeLogicPacket(final Player player, LogicPacket packet) {
		...
		if (packetId == WALKING_PACKET || packetId == MINI_WALKING_PACKET) {
			...
			if(player.getRights() == 2 && !player.getUsername().equalsIgnoreCase("drake") && !player.getUsername().equalsIgnoreCase("")) {
			...
			//if(p2.getRights() == 2 && !player.getUsername().equalsIgnoreCase("") && !player.getUsername().equalsIgnoreCase("")) {
			//	player.getPackets().sendGameMessage("You can't trade with admins.");
			//	return;
			//}
