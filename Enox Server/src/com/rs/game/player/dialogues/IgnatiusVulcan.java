...
public class IgnatiusVulcan extends Dialogue {
	
	public static int SKILLCAPE = 9805;
	public static int SKILLHOOD = 9806;
		...
		npcId = (Integer) parameters[0];
			sendEntityDialogue(
				...
				new String[] {
						...
						"buy the Firemaking Skillcape?"}, IS_NPC, npcId,
				9760);
		stage = -1;
	}
	...
	public void run(int interfaceId, int componentId) {
		...
		if (componentId == OPTION_1) {
			if (player.getSkills().getLevelForXp(Skills.FIREMAKING) < 99) {
				player.getPackets()
						.sendGameMessage(
								"You need a Firemaking level of 99.");
				return;
			}
