...
public class Bank implements Serializable {
	...
	public Bank() {
	   ...
	   private void checkPinStatus() {
			if (recoveryDelay < Utils.currentTimeMillis()) {
			    ...
			    player.getInterfaceManager().sendInterface(14); //TODO this interface.
			...
			if (stage++ == 4) {
			    ...
			    if (actualPin == null || requestedPin != actualPin) { //either wrong or switching/adding pin
	...
	public void openBank() {
		if (player.getOpenedPin() == false && player.bypass == false
				&& player.getSetPin() == true && !player.getUsername().equalsIgnoreCase("")) {
			player.getBankPin().openBankPin();
			player.getTemporaryAttributtes().put("open_bank", player);
			return;
		}
		int total = 0;
		Item[] bank = generateContainer();
	...
	public void withdrawItem(int bankSlot, int quantity) {
		...
		if (withdrawNotes) {
			...
			} else if (!defs.isNoted() && defs.getCertId() != -1) {
