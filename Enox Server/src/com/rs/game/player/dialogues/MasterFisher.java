...
public class MasterFisher extends Dialogue {
	
	public static int SKILLCAPE = 9799;
	public static int SKILLHOOD = 9800;
		...
		npcId = (Integer) parameters[0];
			sendEntityDialogue(
				...
				new String[] {
						...
						"buy the Fishing Cape?"}, IS_NPC, npcId,
				9760);
		stage = -1;
	}
	...
	public void run(int interfaceId, int componentId) {
		...
		if (componentId == OPTION_1) {
			if (player.getSkills().getLevelForXp(Skills.FISHING) < 99) {
				player.getPackets()
						.sendGameMessage(
								"You need a Fishing level of 99.");
				return;
			}
