...
import com.rs.game.player.content.SummoningPouch;
...
public class Summoning implements Serializable {
	...
	public static Familiar createFamiliar(Player player, Pouches pouch) {
		...
		try {
			Familiar fam = (Familiar) Class
					...
					.getConstructor(
							new Class[] { Player.class, Pouches.class,
									WorldTile.class, Integer.TYPE, Boolean.TYPE })
					.newInstance(
			...
			// System.out.println("Familiar not added: "+NPCDefinitions.getNPCDefinitions(pouch.getNpcId()).name.replace(" ",
			// "").replace("-", "")+".");
		}
		return null;
	}
	...
	public static void createPouch(final Player player, int itemId, int amount) {
		...
		for (i = 0; i < amount; i++) {
			...
			double multiplier = 3;
		//	Item[] lame = pouch.getItems();
			player.getInventory().removeItems(pouch.getItems());
		//	player.getInventory().deleteItem(lame, amount);
		//	player.getInventory().deleteItem(itemId, amountToCreate);
	...
	public static void spawnFamiliar(Player player, Pouches pouch) {
		String name = Skills.SKILL_NAME[Skills.SUMMONING].toLowerCase();
		// final ClanWars clanWars =
		// player.getCurrentFriendChat().getClanWars();
		...
		if (player.getSkills().getLevelForXp(Skills.SUMMONING) < pouch
				.getLevel()) {
			player.getPackets().sendGameMessage(
					"You need to have a" + (name.startsWith("a") ? "n" : "")
							+ " " + name + " level of " + pouch.getLevel()
							+ " to summon this.");
			return;
		}
		if (player.getSkills().getLevel(Skills.SUMMONING) < pouch
			...
			player.getPackets()
					.sendGameMessage(
							"You don't have enough summoning points to summon that familiar.");
			return;
		}
		/*
		 * if (clanWars != null && clanWars.get(Rules.NO_FAMILIARS)) {
		 * player.getPackets
		 * ().sendGameMessage("You can't summon a familiar in a NON familiar war."
		 * ); return; }
		 */
