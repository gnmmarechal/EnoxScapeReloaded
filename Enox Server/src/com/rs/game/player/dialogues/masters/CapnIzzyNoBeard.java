...
public class CapnIzzyNoBeard extends Dialogue {
	
	public static int SKILLCAPE = 9772;
	public static int SKILLHOOD = 9773;
		...
		npcId = (Integer) parameters[0];
			sendEntityDialogue(
				...
				new String[] {
						...
						"buy the Agility Skillcape?"}, IS_NPC, npcId,
				9760);
		stage = -1;
	}
	...
	public void run(int interfaceId, int componentId) {
		...
		if (componentId == OPTION_1) {
			if (player.getSkills().getLevelForXp(Skills.AGILITY) < 99) {
				player.getPackets()
						.sendGameMessage(
								"You need a Agility level of 99.");
				return;
			}
