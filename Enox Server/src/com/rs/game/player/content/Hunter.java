	...
	public enum FlyingEntities {
		...
		BABY_IMPLING(1028, 11238, 17, 25, 17, new Item[] { new Item(946, 1),
				...
				YOUNG_IMPLING(1029, 11240, 24, 65, 22, new Item[] { new Item(361, 1),
						...
						ELECTIC_IMPLING(1033, 11248, 56, 289, 50, null, null, null, null, 115),

						SPIRIT_IMPLING(7904, 15513, 63, 321, 54, null, null, null, null, 135),

						NATURE_IMPLING(1034, 11250, 69, 353, 58, null, null, null, null, 155),

						MAGPIE_IMPLING(1035, 11252, 78, 409, 65, null, null, null, null, 175),

						NINJA_IMPLING(6053, 11254, 94, 481, 74, null, null, null, null, 255),

						PIRATE_IMPLING(7845, 13337, 124, 497, 76, null, null, null, null, 320),

						DRAGON_IMPLING(6054, 11256, 150, 553, 83, null, null, null, null, 3600),

						ZOMBIE_IMPLING(7902, 15515, 198, 585, 87, null, null, null, null, 120),

						KINGLY_IMPLING(7903, 15517, 350, 617, 91,
		...
		private FlyingEntities(int npcId, int reward, double puroExperience, double rsExperience, int level, Graphics graphics, Item[] rarleyCommon, Item[] common, Item[] rare, Item[] extremelyRare, int spawn) {
			this.npcId = npcId;
			this.reward = reward;
		...
		private FlyingEntities(int npcId, int reward, double puroExperience, double rsExperience, int level, Item[] rarleyCommon, Item[] common, Item[] rare, Item[] extremelyRare, int spawn) {
			this.npcId = npcId;
			this.reward = reward;
			this.puroExperience = puroExperience;
			this.rsExperience = rsExperience;
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				if (isSuccessful(player, instance.getLevel(), new DynamicFormula() {
					...
					@Override
					public int getExtraProbablity(Player player) {
						/*if (player.getInventory().containsItem(11259, 1) || player.getEquipment().getItem(3).getId() == 11259)
							return 3;//magic net
						else if (player.getInventory().containsItem(10010, 1) || player.getEquipment().getItem(3).getId() == 10010)
					...
					player.getPackets().sendGameMessage("...and you successfully caputure the "+ name2.toLowerCase());
					final WorldTile[] respawn = {new WorldTile(2598, 4319, 0), new WorldTile(2593, 4312, 0), new WorldTile(2584, 4319, 0),
							new WorldTile(2582, 4321, 0), new WorldTile(2589, 4330, 0), new WorldTile(2602, 4326, 0),
							new WorldTile(2602, 4310, 0), new WorldTile(2588, 4309, 0), new WorldTile(2581, 4316, 0),
							new WorldTile(2578, 4316, 0), new WorldTile(2585, 4307, 0), new WorldTile(2599, 4306, 0),
							new WorldTile(2605, 4312, 0), new WorldTile(2610, 4310, 0), new WorldTile(2607, 4325, 0),
							new WorldTile(2599, 4335, 0), new WorldTile(2586, 4336, 0), new WorldTile(2576, 4334, 0),
							new WorldTile(2572, 4332, 0), new WorldTile(2573, 4317, 0), new WorldTile(2576, 4305, 0),
							new WorldTile(2588, 4305, 0), new WorldTile(2588, 4298, 0), new WorldTile(2601, 4298, 0),
							new WorldTile(2614, 4300, 0), new WorldTile(2615, 4315, 0), new WorldTile(2610, 4332, 0),
							new WorldTile(2598, 4342, 0), new WorldTile(2584, 4350, 0), new WorldTile(2569, 4343, 0),
							new WorldTile(2564, 4319, 0), new WorldTile(2565, 4304, 0), new WorldTile(2573, 4292, 0),
							new WorldTile(2603, 4294, 0), new WorldTile(2618, 4293, 0), new WorldTile(2620, 4306, 0),
							new WorldTile(2619, 4323, 0), new WorldTile(2618, 4340, 0), new WorldTile(2604, 4347, 0),
							new WorldTile(2620, 4352, 0), new WorldTile(2623, 4323, 0), new WorldTile(2623, 4308, 0),
							new WorldTile(2620, 4291, 0), new WorldTile(2604, 4289, 0), new WorldTile(2588, 4291, 0),
							new WorldTile(2561, 4298, 0), new WorldTile(2560, 4315, 0), new WorldTile(2562, 4332, 0),};
					final int spawnLocation = Misc.random(respawn.length);
					WorldTasksManager.schedule(new WorldTask() {
						int loop = 0;
						@Override
						public void run() {
							if (loop == instance.getSpawn()) {
								World.spawnNPC(instance.getNpcId(), respawn[spawnLocation], -1, true);
								stop();
							} 
							loop++;
						}
					...
					player.getPackets().sendGameMessage("...you stumble and miss the "+ name2.toLowerCase());
				}
			}
		});
	}
