...
public final class LevelUp extends Dialogue {
	...
	public static void send104m(Player player, int skill) {
		World.sendWorldMessage(
				"<img=9> <col=FFFFFF><shad=00FFFF>[News] " + player.getDisplayName() + " has just reached 104M experience in " + Skills.SKILL_NAME[skill] + "!", false);
		return;
	}
	public static void send200m(Player player, int skill) {
		World.sendWorldMessage(
				"<img=9> <col=FFFFFF><shad=00FFFF>[News] " + player.getDisplayName() + " has just reached 200M experience in " + Skills.SKILL_NAME[skill] + "!", false);
		return;
	}
	@Override
