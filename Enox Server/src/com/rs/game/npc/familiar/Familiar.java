...
public abstract class Familiar extends NPC implements Serializable {
	...
	public void processNPC() {
		if (getDefinitions().getId() == 8422) {
			setHitpoints(owner.getSkills().getAssassinLevel(Skills.ASSASSIN_CALL)*500);
			setCombatLevel(owner.getSkills().getAssassinLevel(Skills.ASSASSIN_CALL)*5);
		}
		if (isDead())
			return;
		unlockOrb();
